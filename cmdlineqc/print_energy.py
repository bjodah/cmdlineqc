...
def main(log_files,
    ...
    if difference:
        assert print_log == False
        print results[0] - unitperserving_sum(results[1:])
    return os.EX_OK

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description=main.__doc__)
    parser.add_argument('-u', '--unit', type=str, default = 'eV', help='Choose from: ' + \
    ...
    parser.add_argument('-p', '--print_log', action = 'store_true', default = False, help='Print logfile name before each energy.')
    parser.add_argument('-i', '--index', type = int, default = -1, help='Index of optimization step (default: last)')
