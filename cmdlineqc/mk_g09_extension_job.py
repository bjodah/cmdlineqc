...
def main(job_files,
    ...
    for job in job_files:
        ...
        if at_exe:
            ...
            new_com = os.path.join(os.path.dirname(com),
                                   unnumname + str(num + 1) + '.com')
        ...
        if from_check:
            ...
            for entry in rootcard:
                if entry.lower().startswith('geom'):
                    if 'connectivity' in entry:
                        continue
                ...
                if entry.lower().startswith('guess'):
                    ...
                    has_guess_read = True
                new_rootcard.append(entry)

            if not has_geom_check:
                new_rootcard += ['geom=allcheck']
            if not has_guess_read:
                new_rootcard += ['guess=read']

            new_chk = os.path.join(os.path.dirname(job),
                                   unnumname + str(num + 1) + '.chk')
            assert not os.path.exists(new_chk)
            shutil.copy(src_check, new_chk)
            # Write new com-file (let other script set % variables)
            ofh.write('#' + ' '.join(new_rootcard) + '\n\n')
        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-l', '--from-log', action='store_true',
                        help='Grab the com with same name as provided'
                        ' log-file(s)')
    ...
    parser.add_argument('-c', '--from-check', action='store_true',
                        help='Start from checkpoint file instead of'
                        ' reading geometry')

    parser.add_argument('job_files', nargs='+', type=str,
                        help='com- or log-files')
